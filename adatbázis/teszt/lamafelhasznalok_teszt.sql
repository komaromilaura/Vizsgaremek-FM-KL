...
-- Adatbázis struktúra mentése a lamafelhasznalok_teszt.
DROP DATABASE IF EXISTS `lamafelhasznalok_teszt`;
CREATE DATABASE IF NOT EXISTS `lamafelhasznalok_teszt` /*!40100 DEFAULT CHARACTER SET utf8mb3 COLLATE utf8mb3_hungarian_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `lamafelhasznalok_teszt`;

-- Struktúra mentése tábla lamafelhasznalok_teszt. admin
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.admin: ~0 rows (hozzávetőleg)
INSERT INTO `admin` (`IVIR`, `Password`, `Admin`) VALUES
	(22222, 'MTIxMg==', 0),
	(101301, 'MTIxMg==', 1);

-- Struktúra mentése tábla lamafelhasznalok_teszt. allashelyek
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.allashelyek: ~3 rows (hozzávetőleg)
...
-- Struktúra mentése tábla lamafelhasznalok_teszt. allomasok
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.allomasok: ~23 rows (hozzávetőleg)
...
-- Struktúra mentése tábla lamafelhasznalok_teszt. belso_ell
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.belso_ell: ~4 rows (hozzávetőleg)
...
-- Struktúra mentése tábla lamafelhasznalok_teszt. beszerzesek
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.beszerzesek: ~13 rows (hozzávetőleg)
...
-- Struktúra mentése tábla lamafelhasznalok_teszt. dolgozok
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.dolgozok: ~4 rows (hozzávetőleg)
...
-- Struktúra mentése tábla lamafelhasznalok_teszt. kulso_ell
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.kulso_ell: ~3 rows (hozzávetőleg)
...
-- Struktúra mentése tábla lamafelhasznalok_teszt. kulso_ellenorok
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.kulso_ellenorok: ~2 rows (hozzávetőleg)
INSERT INTO `kulso_ellenorok` (`ID`, `nev`, `ir_szam`, `varos`, `kozterulet`, `kozt_jellege`, `hazszam`, `epulet_emelet_ajto`, `helyrazi_szam`, `email`, `telefon`, `kontakt_szemely`) VALUES
	...
	(2, 'Vas Megyei Kormányhivatal Sárvári Járási Hivatal Járási Népegészségügyi Intézete', 9600, 'Sárvár', 'Várkerület', NULL, '4.', NULL, NULL, 'titkarsag.sarvar@nydr.antsz.hu', '06 95 320 277', 'Szalai Márton'),
	(3, 'Állami Számvevőszék', 1052, 'Budapest', 'Apáczai Cs. J.', 'utca', '10.', NULL, NULL, 'szamvevoszek@asz.hu', '06 1 484 9100', 'Takács Bence');

-- Struktúra mentése tábla lamafelhasznalok_teszt. letszam
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.letszam: ~5 rows (hozzávetőleg)
...
-- Struktúra mentése tábla lamafelhasznalok_teszt. megyek
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.megyek: ~3 rows (hozzávetőleg)
INSERT INTO `megyek` (`ID`, `megye_nev`) VALUES
	(1, 'Vas'),
	(2, 'Győr-Moson-Sopron'),
	(3, 'Zala');

-- Struktúra mentése tábla lamafelhasznalok_teszt. migransellatas
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.migransellatas: ~9 rows (hozzávetőleg)
...
-- Struktúra mentése tábla lamafelhasznalok_teszt. mozgoorseg
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.mozgoorseg: ~3 rows (hozzávetőleg)
...
-- Struktúra mentése tábla lamafelhasznalok_teszt. munkakorok
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.munkakorok: ~9 rows (hozzávetőleg)
INSERT INTO `munkakorok` (`ID`, `munkakor`, `alapber`) VALUES
	...
	(9, 'állomásvezető', 660000);

-- Struktúra mentése tábla lamafelhasznalok_teszt. partnerek
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.partnerek: ~6 rows (hozzávetőleg)
...
-- Struktúra mentése tábla lamafelhasznalok_teszt. tulorak
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.tulorak: ~4 rows (hozzávetőleg)
...
-- Struktúra mentése tábla lamafelhasznalok_teszt. users
...
-- Tábla adatainak mentése lamafelhasznalok_teszt.users: ~6 rows (hozzávetőleg)
